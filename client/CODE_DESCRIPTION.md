...
- `scripts` expose Vite development, build, preview, lint, and automated
  formatting commands used during development; `npm run format` runs Prettier on
  the source tree.
...
- Provides the root element (`<div id="root" />`) where the React tree mounts.
- Loads the JavaScript entry module (`/src/main.jsx`) via `<script type="module">`;
  styles are now bundled directly inside the client rather than loaded from a CDN.

## `src/main.jsx`

- Imports React and the DOM renderer entry points.
- Loads Bootstrap's stylesheet and the local animation helpers so they are
  bundled with the SPA instead of relying on external CDNs.
...
- Defines CSS custom properties that centralise animation timing and easing.
- Provides utility classes (`fade-in`, `slide-up`, `scale-on-hover`, `todo-item`)
  used across the SPA to animate layout sections, interactive elements, and todo
  list entries.
- Declares keyframes for fading and sliding content along with hover and
  completion transitions that React components consume via class names.
...
- Imports React state and effect utilities alongside React Router components and
  React-Bootstrap primitives such as `Navbar`, `Fade`, `Spinner`, and `Alert` to
  orchestrate layout and animations.
- Pulls the `useAuth` hook to access authentication data, and reuses the shared
  `AuthForms`, `TodoDashboard`, and `RegisterPage` components plus the API
  helpers for session management.
- `AppShell` component:
  - Provides a consistent layout containing the navbar, optional status messages
    (rendered inside a `Fade` so they animate in/out), and a loading spinner that
    fades while the app bootstraps.
  - Applies the CSS utility classes (`fade-in`, `slide-up`, `scale-on-hover`) to
    animate navigation, alerts, and routed views.
...
- `App` component:
  ...
  - Manages `bootstrapping` (initial loading state) and `statusMessage` for
    global feedback, automatically clearing messages after four seconds via an
    effect.
...
- Imports React state hooks, the React Router navigation helper, and
  React-Bootstrap form components including `Fade` for animated feedback.
...
- `handleSubmit` prevents the default form submission, toggles the loading state,
  ...
  via `onLogin` before navigating to `/todos`; errors fade in/out inside an
  `<Alert>`.
- Renders a card enhanced with the animation utility classes so the panel slides
  upward and scales on hover, with conditional fields and button labels depending
  on whether the component operates in login or registration mode.
...
- Imports React state/effect hooks and React-Bootstrap components for layout,
  forms, lists, alerts, fades, and spinners to provide animated feedback.
...
- `handleDelete` removes the todo both on the server and locally when deletion
  succeeds.
- Renders a card containing the add form, error alert (fading in/out), loading
  indicator with spinner, and a list group where each todo item animates into
  view, highlights completion with transitions, and exposes a delete button; also
  renders an empty state message when there are no todos.
...
- Wraps the form with an introductory card that highlights the benefits and
  ...
  aspettarsi prima di compilare i campi; both the card and list items use the
  shared animation classes so the onboarding content gently slides into view.
